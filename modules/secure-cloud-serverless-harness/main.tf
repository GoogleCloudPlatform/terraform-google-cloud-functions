...
locals {
  ...
  decrypters = join(",", concat(["serviceAccount:${google_project_service_identity.artifact_sa.email}", "serviceAccount:${data.google_storage_project_service_account.gcs_account.email_address}"], var.decrypters))
  encrypters = join(",", concat(["serviceAccount:${google_project_service_identity.artifact_sa.email}", "serviceAccount:${data.google_storage_project_service_account.gcs_account.email_address}"], var.encrypters))
...
resource "google_project_iam_member" "serverless_sa_roles" {
...
module "cloudfunction_source_bucket" {
  ...
  project_id    = module.serverless_project.project_id
  name          = "bkt-${var.location}-${module.serverless_project.project_number}-cfv2-zip-files"
  location      = var.location
...
resource "google_sourcerepo_repository" "serverless_source_repo" {
  count   = var.serverless_type == "CLOUD_RUN" ? 0 : 1
  name    = "rep-secure-serverless"
  project = module.serverless_project.project_id
